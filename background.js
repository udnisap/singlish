...
function enalble() {
    chrome.browserAction.setIcon({path: "icon-on.jpg"});
    chrome.browserAction.setTitle({title: "Click to disable Singlish Translator"});
...
function disable() {
    chrome.browserAction.setIcon({path: "icon-off.jpg"});
    chrome.browserAction.setTitle({title: "Click to enable Singlish Translator"});
...
function callBack(info, tab) {
    if (info.selectionText.length > 0) {
        //pass message to change the text
        var translation = translator.translate(info.selectionText);
        updateContent(info.selectionText, translation);
    }
}

function updateContent(original, replace) {
    chrome.tabs.getSelected(null, function(tab) {
        chrome.tabs.sendMessage(tab.id, {original: original, replace: replace, type: "bg2c"});
    });
}
chrome.extension.onMessage.addListener(
        function(request, sender, sendResponse) {
            if (request.type == "c2bg" && enabled)
                sendResponse({original: request.text, replace: translator.translate(request.text)});
        }
);


/* 
 * Translator bassed on the UCSC UNICODE Transaltor
 */
